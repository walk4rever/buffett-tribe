...
| `/company` | 公司目录与数字价值线研究画布（全量 16,435 家标的极速模糊检索、未建档标的 Apple 风格快速通道优先建档、`/company/[id]` 默认呈现 DVL 旗舰视图） |
...
| `/admin/universe` | 公司大盘管线与全市场 16,435 家标的探查看板（Phase 0/1/2 宏观进度、快速通道加急队列、三大市场分布、最近建档动态、死信预警、多维检索） |
...
工具调用有实时指示器，显示工具名、参数摘要、返回条数。工具跑完到正文出现之间模型往往还要推理十几到几十秒，这段时间界面会显示「正在思考…」/「正在分析查询结果…」的等待指示，不会看起来像卡死。公司页右下角还有一个直接锚定当前公司的 "AI 解读" 悬浮入口。

复杂问题可能要跑几十次工具调用、耗时一两分钟，这是正常的。万一回答确实被中断（网络断开等），界面会明确标出"回答未完整结束"并提示可以直接说"继续"——已经生成的部分会保留，接着往下答即可，不用重问。
...
### /company — 数字价值线（DVL）研究画布

公司详情页面（`/company/[id]`）默认呈现数字价值线（Digital Value Line）旗舰卡片与 7 Tab 结构化深度投研流：业务分析 · 财务分析 · 价值分析 · 管理分析 · 估值分析 · 大师持仓 · 参考资料。

支持美股（SEC EDGAR）、港股（HKEXnews）、A 股（巨潮资讯网）三大市场统一规范 Slug（`us-0001652044`、`hk-09992`、`cn-600519`），原生支持多 Ticker 股票类别（如 Alphabet Class A/C）就地实时切换。

数据来自两层：
1. **Fact 层**：核心数字价值线卡片（现价、52周区间、历史 PE/PB 估值中枢与 5 年拟合通道）、巴菲特价值体检精密四宫格（支持银行保险/公用事业/房地产/强周期资源等特殊行业财务模型深度自适应与周期排雷）、13F 大师持仓穿透聚合（多类别合并与拆解明细）、财务矩阵（三大报表 5-10 年展开、年度/单季切换、4 季 TTM 动态追踪、股票回购序列）、历史价格走势（日K/周K/月K/季K、复权切换）、参考资料（三大市场年报、10-Q、中报、季报原文归档至 R2，支持 HTML/PDF 双引擎阅读器与「AI 解读」浮窗交互）
2. **生成层**：LLM 生成的公司概览、业务概览、九宫格商业模式画布、护城河雷达分析、管理层配置分析、估值安全边际分析

### /master — 大师 / 投资人

每位投资人的独立主页：13F 持仓快照、季度点评。3 位核心"大师"额外拥有完整原文材料库（可全文阅读），另外 10 位是仅追踪 13F 持仓的 Alpha 部落投资人。
...
- `/company/[id]` 公司研究画布，覆盖美股 / 港股 / A 股三个市场；576 家公司已完成上线（全库 Phase 1 达成率达 97.6%），目录页严格以 `metadata.onboardPhase >= 1` 为准绳，待完善标的精准收敛至 14 家非经营性特殊标的（破产/SPAC/基金等）；**页面已升级为 8-Tab 统一工作区**（价值线为第一 Tab，常驻 Header 显示股价/市值/PE/PB，原深度投研 7 Tab 依次跟随）；Onboard 自动化接入管线原生支持 SEC 官方挂牌探查与 10-Q 季度财报 / S-1 招股书级联降级
...
- 13F / 10-K / 20-F / 40-F 批处理导入 + 招股书/10-Q 降级导入 + 港股披露易 / A 股巨潮资讯网年报导入
