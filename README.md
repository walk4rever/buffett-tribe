...
数据来自两层：
1. **Fact 层**：核心数字价值线卡片（现价、52周区间、历史 PE/PB 估值中枢与 5 年拟合通道）、13F 大师持仓穿透聚合（多类别合并与拆解明细）、财务矩阵（三大报表 5-10 年展开、年度/单季切换、4 季 TTM 动态追踪）、历史价格走势（日K/周K/月K/季K、复权切换）、参考资料（三大市场年报、10-Q、中报、季报原文归档至 R2，支持 HTML/PDF 双引擎阅读器与「AI 解读」浮窗交互）
