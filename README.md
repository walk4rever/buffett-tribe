...
- `/agent` 投资研究 Agent，五工具上线，工具调用有实时指示器；公司页额外有独立锚定当前公司的 "AI 解读" 入口；所有 AI 解读对话框均支持剪贴板粘贴图片提问（DeepSeek vision）；登录用户对话按页面/投资人/公司维度持久化（含图片），冷启动时 Agent 会回放历史，不会对已聊过的内容"失忆"
