...
0. [核心准则](#核心准则) ← 最高准则，其余章节与之冲突时以本节为准
1. [产品定位](#产品定位)
...
> 本节是这份文档的最高准则。以下任何路线图、架构决策、设计规范与本节冲突时，以本节为准。

### 买股票就是买公司

价值部落存在的意义，是帮助用户更好地理解一家公司，以便做出更好的投资决策。**理解公司，是 kernel of core。** 其余一切——数据管道、Agent、页面、设计——都只是为这一件事服务。

### 理解公司的三个层次

**第一层 · 事实**——公司所有基于事实的基本信息：财报、业绩、业务模式、公司自己披露了什么。这一层可核验，唯一标准是准确且可追溯到原始披露。对应 `Financial` / `FilingSection` / `CompanyAnalysis.profile,business`；L4 数据完整性检查守的就是这一层。

**第二层 · 框架**——以巴菲特为代表的投资大师的思维框架：护城河在哪里？管理层可信吗？现在的价格有安全边际吗？对应 `CompanyAnalysis.moat/management/valuation`，以及两个佐证面：大师说过什么（`search_wisdom`）、大师实际持有什么（`Holding`）。这一层的价值不在结论本身，**在于结论必须挂在第一层的事实上**——判断若不引用具体财务/披露证据，就退化成模型的一般知识，等于没有。

**第三层 · Situational awareness（变化）**——公司的经营与外部环境每时每刻都在变化。利用 AI 等技术，围绕一家公司持续接收各种信息，**识别信号、排除噪声**，让用户理解公司的现状。

第三层的难点不在信息接入量，在信号判据：

- **信号的判据不是通用新闻价值，而是第二层的框架。** 一条信息是不是信号，只看它是否改变了我们对这家公司护城河 / 管理层 / 估值中某一格的判断。新闻聚合器按热度排序（通用注意力经济），我们按"是否动摇既有判断"排序（投资决策的注意力经济）。同一条"公司换 CFO"的消息，在一家已判定"管理层资本配置纪律是核心变量"的公司上是强信号，在别处就是噪声。**只有已经持有这家公司框架状态的系统才做得出这个判断**——而这正是第一、二层沉淀下来的东西。
- **需要一份可比较的"当前理解"状态。** 现在 `CompanyAnalysis` 是覆盖式重写，没有"上一次我们认为什么"的可 diff 表达。第三层要成立，这份状态必须可引用、可挑战、可标记为已过期。
- **每条进来的信息要被路由到某一格并给出裁决**：确认既有判断 / 削弱既有判断 / 提出新变量 / 挂不上任何格子。第四类即噪声，直接丢弃，不进用户视野。
- **每条信号必须带证据链**：原文出处 + 对应的框架格子 + 为什么这构成改变。缺任一项就是模型在讲故事，比不给更有害。
- **默认姿态是宁缺毋滥。** 一家公司一个季度可能只有 0–3 条真信号，系统敢于长期沉默才是对的。把噪声当信号推给用户比没有这一层更糟——它消耗用户最稀缺的注意力，并训练用户不再信任它。这与"每天都要有内容推送"的产品直觉冲突，冲突时以本条为准。

现状（2026-08-28）：第一、二层已有实体承载，第三层基本为空，且现有数据的时间粒度本身就给不出这一层——13F 是季度快照且滞后 45 天；年报是年度文档，`FilingSection` 只存当期切片，跨年 diff（例如"今年风险因素新增了哪一条"）查不出来；价格数据未与基本面 / 披露 / 持仓的变化对齐；Agent 与生成脚本都还缺真实 web_search 能力。内部来源（年报跨期 diff、13F 变动、价格与基本面背离）现在就能做且干净；外部来源（公告、管理层言论、行业数据、新闻）需要先补检索能力。

### AI / Agent 是手段，不是目的

AI 是当前最好的手段之一，仅此而已；有更好的技术就持续引入与切换。不变的是"买股票就是买公司、理解公司"这个核心。落到工程上有四条不能松的约束：

1. **唯一真相是原始披露，不是模型输出。** `Financial` / `FilingSection` / `Holding` / `ExtSource` 来自 SEC、港交所、巨潮，换十次模型都不变。`CompanyAnalysis` 五格全是可再生的派生物——任何时候一份生成内容都必须能删掉重跑，不能变成"只有模型知道、删了就没了"的原始数据。
2. **证据链是模型无关的，这是抗技术更替的关键。** 每条判断都要带 `evidence` + `sourceRef`（如 `10-K FY2025 Item 5`、`13F 2026Q1`）并禁止编造数字（`generate-management-analysis.ts` 已是这个做法，五格都应如此）。`sourceRef` 指向原始披露而非模型记忆，所以换模型时能验证的是"新模型在同一份证据上判断是否更好"，而不是"听起来是不是更聪明"。没有证据链，就没有换模型的标尺，只能靠感觉换。
3. **供应商是配置，不是架构。** 生成脚本走 `AI_API_KEY` / `AI_API_BASE_URL` / `AI_MODEL`，Agent 走 pi-gateway，都停留在环境变量层面；不把任何供应商特性写进数据模型或产品逻辑。
4. **衡量标准是理解质量，不是输出观感。** 不是"Agent 回答得像不像人""文章读起来漂不漂亮"，而是"用户对这家公司的理解是否更准确、更及时"。评估应建在公司级别：判断是否有证据支撑、证据是否指向真实披露、变化是否被及时捕捉。

### 界面是手段的手段

公司页、`/agent` 对话都只是当下的界面形态，不是产品的定义。界面存在的意义是帮助用户理解公司——哪种更有效就用哪种，可以换、可以并存、可以推倒；产品形态在迭代中试，不预先锁定（第三层的形态尤其如此：时间线 / 推送 / 对话内主动提示都还没有定论，先把"什么算信号、挂在哪一格、证据是什么"定清楚）。

因此真正需要沉淀、需要经得起换的是界面背后的三层（事实 / 框架 / 变化），它们是形态无关的。界面应当是薄的、可替换的一层。反例：把判断逻辑或证据链只存在于某个页面组件的渲染里——那样换一次形态就要重做一次。

### 用作判断标尺

新增或改动任何东西时，按这个次序自问：

1. 它让用户对公司的理解好在哪里？说不清就不做。
2. 它属于哪一层（事实 / 框架 / 变化）？跨层的结论是否挂回了下一层的证据？
3. 它是不是把手段当成了目的——为用 AI 而用 AI、为做界面而做界面？
4. 换掉模型、换掉界面形态时，它要重做多少？要重做的部分是否本该沉淀在下面三层里？
...
> **品牌改名（2026-08-30，v0.44.0）**：巴菲特部落 · Buffett Tribe → **价值部落 · Value Tribe**。改名的直接动因是规划面向美国市场的英文支线——一个品牌两个 locale，同时规避在美国用 Buffett 的名字做商业产品的商标／姓名权风险。用户可见的品牌名统一收敛到 `src/lib/brand.ts`（pi-gateway 因是独立 deployable，另有一份 `services/pi-gateway/src/brand.ts`，两者需手动同步）。
>
> **内部标识符一律不改**（改了会破坏线上数据或部署）：R2 key 前缀 `buffett-tribe/...`、PM2 进程名 `pi-gateway-buffett-tribe`、`package.json` 的 `name`、`/api/mcp` 的 MCP server `name`、以及全部域名——**域名迁移尚未处理**，站点当前仍在旧域名下运行。英文支线本身（locale 骨架、文案抽取、生成内容的 locale 维度）也尚未开工，完整分期方案见 `handoff.md`「美国市场支线的完整方案」。
...
- **投资人（Filer/tribe member）**：价值部落核心成员（有完整 wisdom 库）不超过 10 个；Alpha 部落（13F-only）多一些，顶级的也就数十个，**投资人总数上限在 100 个以内**——稳定上限。
...
**投研笔记 + 个人工作区侧栏（2026-08-28 上线）**：`/agent` 页面改成左中两栏——左侧「个人工作区」侧栏，右侧是对话区，笔记打开时**原地替换**对话区（不是浮窗、不是第三栏）。侧栏是手风琴结构（`WorkspaceSidebar.tsx` + `CollapsibleSection.tsx`），当前两个分区：**笔记本**（默认展开——数据模型/API/交互仍是"投研笔记"这套设计，只是 UI 标签改短了）、**资料库**（默认收起，占位态"构建中——未来支持在这里上传 PDF/PPT 等资料，作为对话的临时参考"）。资产组合 Portfolio 当天晚些时候也上线了，但没有并入这个手风琴，见下一段——**关注公司列表最终也落在了右侧手风琴里，不是这里**，计划变更记录同样见下一段。
...
**打开笔记默认是预览模式**（不是编辑模式）——`NoteEditor` 的 `mode` state 初始值是 `"preview"`；`AgentPageChat.tsx` 给 `<NoteEditor>` 加了 `key={activeNote.id}`，切换到另一条笔记时强制重新挂载组件、把 `mode` 重置回默认值，不会带着上一条笔记停留的编辑/预览状态过去。预览态的字号/行距也顺带调宽（`font-size:1rem; line-height:1.85`，此前继承 `body` 的 `1.47`，读起来偏挤）。**支持剪贴板粘贴图片**（`onPaste` 挂在 textarea 上，只在编辑模式可用）——复用聊天输入框同一条下采样流水线（`fileToImageAttachment`，长边 1280px、JPEG 0.82），上传到新路由 `POST /api/notes/images`（`buildUserObjectKey(userId,"note-images",...)`，与聊天贴图的 `chat-images` 分类同规则不同前缀），成功后把 `![](url)` 插入光标位置，走已有的自动保存管线——不是像聊天那样先内联 base64、发送时才转存 R2，笔记场景没有"消息发送"这个动作可以挂钩，粘贴当下就直接传。mime 分类到扩展名的映射从 `agent-turns` 路由里抽成 `src/lib/image-attachment.ts` 的 `imageExtensionForMimeType()` 共享，两个上传路径不再各自维护一份。
...
**同日追加，用户反馈后的三处修复 + 关注列表占位**：① 左右两侧栏与对话区之间的分界线太淡（`0.5px #e8e8ed` 在页面浅色渐变背景下几乎看不见）——`.agent-workspace-sidebar`/`.agent-workspace-sidebar-right` 的 `border-right`/`border-left` 改成 `1px rgba(0,0,0,0.14)`。② Portfolio 面板要求可折叠——`PortfolioPanel.tsx` 改成整体用 `CollapsibleSection` 包裹（原来是自己手写的头部+按钮，不参与手风琴）。③ 用户追问"资产组合的折叠按钮/文字是否应该镜像到右边，跟笔记本的按钮在左边对称"——认可后给 `CollapsibleSection.tsx` 加了 `align?: "left"|"right"` prop（默认 `"left"`，不影响左侧栏三处既有调用），`"right"` 时 `.agent-workspace-section-head`/`.agent-workspace-section-toggle` 用 `flex-direction: row-reverse`（flex 主轴反向后 `justify-content` 默认值 `flex-start` 自然把 chevron+标题这组内容推到面板的外侧/右边缘，不用手写 `justify-content:flex-end` 加反转 DOM 顺序两步）——效果是右侧栏 chevron/标题贴右边缘、"+添加"按钮在其左侧，跟左侧栏"chevron/标题贴左边缘、按钮在右侧"形成真正的镜像，不是同一套布局简单搬到右边。④ **新增「关注列表」占位分区**（`WatchlistSection.tsx`，纯占位文案"构建中——未来支持维护你关注的公司列表，快速跳转到对应研究页面"，没有假按钮，同「资料库」的克制）——**这是一次计划变更**：`TODO.md` 原先"个人工作区四模块"的构想里，关注公司列表排在**左侧**手风琴的第三个分区；这次用户直接要求它跟 Portfolio 一起放在**右侧**，新建 `RightWorkspaceSidebar.tsx`（`agent-workspace-sections` 容器包 `PortfolioPanel` + 关注列表两个 `CollapsibleSection`，均 `align="right"`，默认展开/收起状态照抄左侧"笔记本展开/资料库收起"的先例），`AgentPageChat.tsx` 右侧 `<aside>` 现在渲染这个新组件而不是直接渲染 `PortfolioPanel`。⑤ **顺手修的真 bug**：用户测试添加 `00700.HK` 显示"暂无价格数据"——排查发现站内 HK ticker 一律存成 4 位零填充（腾讯是 `0700.HK`，不是 `00700.HK` 或 `700.HK`），`normalizeTicker()`（`src/lib/ticker.ts`，全站约 15 处调用，onboarding/13F/`getCompanyByTicker` 等共用）原来没做这层归一化——加了一段正则把 HK 代码的前导零剥掉再补齐到 4 位，验证过对已入库的正确格式是幂等的（不会破坏任何现有匹配）。**验证**：`tsc`/`lint`/`build` 全绿；Playwright 用临时测试账号验证过分界线截图、折叠展开互相独立、镜像布局截图、`00700.HK` 正确显示"TENCENT HOLDINGS LIMITED"+真实现价，测试账号均已清理。

**同日追加，货币归一化 + 左右视觉一致性**：用户提出两个问题，均是真问题：① A股怎么加（`600519.SS`/`300750.SZ` 格式，`.SH` 不识别——纯答疑，未改代码，参考 HK 的 4 位零填充逻辑）；② **汇总是否考虑了货币差异——答案原来是"没有"，是个真 bug**：`StockPrice.close` 是各证券自身交易货币的原始报价（Yahoo Finance 源，从不做汇率换算），但汇总此前直接把 USD/HKD/CNY 持仓的市值加在一起、套一个硬编码的 `$` 前缀——0700.HK 那笔仓位实际是 HK$，却显示成好像是美元。修复：`currencyForTicker()`（`src/lib/ticker.ts`，从 ticker 后缀推断，`.HK`→HKD、`.SS`/`.SZ`→CNY、其余→USD，不用查库）+ `currencyPrefix()`（`src/lib/currency.ts`，导出原本私有的 `CURRENCY_PREFIX` 表，USD 沿用站内"无前缀"的既有惯例，CNY `¥`/HKD `HK$` 复用公司页财务数字的同一套符号）。`PortfolioHoldingDto` 加 `currency` 字段；汇总按货币分组分别展示子合计（`summarizeByCurrency()`，不做汇率换算合并成一个数字——没有可靠汇率源，装作能合并比明确分组更误导人）；每行的成本/现价/市值/盈亏都带上对应货币前缀，另外在 ticker 旁加一个 `USD`/`HKD`/`CNY` 文字标签（`.agent-portfolio-currency-tag`）——USD 因为没有前缀符号，光靠数字本身完全看不出币种，必须有这个标签才算"标注清楚"。同时把成本行加了"现价（日期）"（用户要求"成本后面加价格与日期"），方便一眼看出这个市值是不是用了过期报价。**顺带做的一致性审查**：用户追问"左右两边字体样式是否统一"——查出来真不统一：持仓行的 ticker（0.9rem/600）、meta 行（0.78rem）都比左侧笔记列表的对应角色（`.agent-workspace-note-title` 0.82rem/500、`.agent-workspace-note-snippet` 0.72rem）大一号；持仓行也没有笔记列表项那种 hover 高亮反馈（明明同样可点击）。统一改成对齐左侧的字号阶梯（ticker 0.82rem、meta/公司名/无价格提示 0.72rem、新增的价格日期 0.68rem 对应 `.agent-workspace-note-date`），字体本身两边一直共用页面级 `--font-sans`、没有过分歧；持仓行加 `:hover{background:var(--secondary)}` + 8px 圆角，和笔记列表项的 hover 行为对齐。汇总区的大号数字（1.3rem）留作右侧独有的"总览"样式，左侧没有对应角色可比，不强行统一。**验证**：`tsc`/`lint`/`build` 全绿；Playwright 用临时账号加了 AAPL(USD)/0700.HK(HKD)/600519.SS(CNY) 三种货币持仓截图确认——三个货币分别显示正确的子合计和符号，行内成本/现价/日期/货币标签均正确渲染，测试账号已清理。
...
### 规模化演进：数字价值线（Digital Value Line）与卡片翻阅闭环（2026-09-21 确立）

#### 1. 背景与核心破局点

在系统从追踪十几位大师扩展到 300+ 家持仓公司的过程中，传统“全量手工深度 Onboarding 管线”（逐家下载数十兆年报、切块归档 R2、调用 5 次 LLM 生成深度研报）撞上了**严重的产能与成本天花板**。13F 导入是指数级扩张的，而重型研报生成却无法线性扩展，导致大量标的堆积在“待完善”状态。

**破局核心思想——重现巴菲特的“价值线（Value Line）翻阅法”**：
巴菲特当年寻找伟大公司的核心方法，不是读厚重的分析报告，而是坐在办公室里将数千页的《价值线投资调查》（Value Line Investment Survey）与《穆迪手册》从 A 到 Z 一页一页翻过去。Value Line 的威力在于**极致的标准化与高信息密度**：每一家公司被严格压缩在一页纸内，包含标准化价格与估值走势图、10 年核心财务报表与比率。这种统一的视觉格式让大脑能在 3 秒钟内快速过滤掉平庸或风险公司，识别出异常优秀的价值机会，随后再针对性调阅年报深入研究。

价值部落从本阶段起，将公司层全面重构为 **“AI 时代的数字价值线（Digital Value Line）”**。

#### 2. 双层漏斗架构（Two-Tier Funnel）

彻底打破“每家公司都必须一开始做成完整大厦”的重型思维，建立极速客观数据与按需 AI 智能的双层漏斗：

```text
┌────────────────────────────────────────────────────────────────────────┐
│  第一层：万家公司的“价值线卡片”（Value Line Card）                   │
│  - 中港美全市场覆盖（万家公司平等收录）                                │
│  - 纯客观数据基座：5~10 年复权价格与利润走势图 + 价值投资体检核心三围 │
│  - 零大模型成本，纯数据流批量同步，毫秒级流式呈现                      │
└───────────────────────────────────┬────────────────────────────────────┘
                                    │
                  用户翻阅时识别出机会：“这家公司有意思！”
                                    │
                                    ▼ 点击【AI 深度研读】
┌────────────────────────────────────────────────────────────────────────┐
│  第二层：AI 专属分析师跟进深度拆解（On-demand Deep Dive）              │
│  - 商业模式九宫格画布 + 护城河壁垒评级                                  │
│  - 管理层资本配置行为 + 多情景估值安全边际                              │
│  - 原文年报精准查证                                                    │
│  - 一旦触发生成，结果永久落库 Supabase，全站共享成果                   │
└────────────────────────────────────────────────────────────────────────┘
```

#### 3. 价值线卡片（Value Line Card）与真·价值线通道设计

单张卡片作为独立、高密度的“原子投研组件”，3 秒内传递完备的核心事实与决策线索：

1. **头部看板与定价三围（Identity & Pricing）**：
   - 公司中文名/英文名、Ticker、交易所、行业（Sector）。
   - 当前股价、总市值、市盈率 PE、市净率 PB。
   - **估值状态角标**：`🟢 处于价值击球区 (折价 ~X%)` / `🟡 合理估值中枢` / `🔴 处于溢价高估区 (溢价 ~X%)`，一眼看穿错杀机会。
2. **公司概览（Company Overview，置顶替代原商业本质单行框）**：
   - 整合“公司基本盘与定位”与“主营产品与营收结构”为单一标准卡片；
   - 深度讲透：① 公司是什么（成立背景、总部、行业定位与软硬件一体生态）；② 主打什么产品、主营业务收入是什么（最新分部营收占比、毛利率中枢与核心驱动力）。
3. **大师持仓小卡片（Master Holdings Mini Cards）**：
   - 聚焦展示：**投资人姓名**（沃伦·巴菲特、李录、段永平等）、**仓位占比**（如 `22%`）、**持仓金额**（如 `$576.4亿`）与报告季度；
   - **长基金公司名称规范处理**：自动净化 `Inc` / `LLC` / `L.P.` 等法言冗余，底部单行截断（`text-overflow: ellipsis`）永不破框，鼠标悬停 Tooltip 完整展示原生全称。
4. **真·价值线走势图（The Value Line Corridor）**：
   - **蓝色实线 + 渐变阴影**：过去 5 年复权股价走势（Catmull-Rom 三阶平滑样条）。
   - **橙色虚线**：**每股业绩价值参考线**（`EPS(t) × 历史中枢估值倍数`），实至名归重现 Arnold Bernhard 原版价值线精髓。
5. **巴菲特价值体检四宫格（The Buffett Quadrant）**：
   - **5年 ROE 均值与稳定性**：5 年平均 ROE + 最低值 + 护城河评级（`★ 卓越护城河` / `稳健盈利` / `周期波动`）。
   - **真金白银造血力**：`经营现金流 / 净利润`（识别纸面利润 vs 真实真金白银）。
   - **资本分配（Capital Allocation）**：**5 年总股本缩减率 %**（`🔥 5年注销回购 -14.2%` vs `⚠️ 5年增发稀释`）+ 5年回购累计金额。
   - **资产负债安全性**：`总负债 / 总资产` + 净现金状态（`手握净现金 / 低负债`）。
6. **复合增速与默认常驻展开的年报财务矩阵**：
   - **5 年营收与净利润复合增速 (CAGR)**。
   - **详细年报表默认直接展开**：移除折叠交互按钮，5~10 年核心科目常驻展示；移动端支持横向触控滑动且**首列科目名称自动粘滞吸顶（Sticky Column）**。
7. **AI 深度透视两极研判（Moat & Weakest Link）**：
...
- 年报整本 PDF 下载与 Cloudflare R2 归档**不再作为前置阻塞条件**，独立为可选的“进阶沉浸式阅读”阶段。
- 未归档 R2 的年报在卡片上直接挂官方源直链；只有核心标的或用户显式要求沉浸式阅读时，才触发后台异步下载与 R2 转存。
...
### v0.45.21 变更（2026-09-26）

- **数字价值线（DVL）核心估值与股本计算 Bug 彻底根治（P0）**：
  - **股本推算健壮化**：支持负净利与负 EPS 安全相除推算正股本，最新年份微亏/停牌时自动平滑回溯借用历史有效股本（如我爱我家 `000560.SZ` 市值从空恢复为 ¥86.26 亿、PB 0.9x、英皇国际 `0163.HK` 市值从空恢复为 HK$5.86 亿、PB 0.04x）；
  - **彻底打破基准市盈率（Benchmark PE）循环论证**：改用严格的历史 5 年 PE 中位数（结合 ROE 中枢在 10~38x 之间合理截断约束），恢复价值走廊击球区/折价真实研判能力（如可口可乐 `KO` 基准 PE 恢复为历史中位数 24.3x，当前 29x 真实呈现溢价 +19% 贴近合理中枢上限，不再虚假呈现 0%）；
  - **跨类股（Share Class）估值对齐**：针对伯克希尔（`BRK-B` vs `BRK-A`）等跨类股标的，引入智能 EPS 锚点探查与 1:1500 比例换算，市值从荒谬的 $6.48 亿美元恢复为真实的 $1.09 万亿美元、市盈率从 0.0x 恢复为 16.4x；同时拦截异常极端 PE；
  - **5年复合增速（CAGR）语义状态自适应**：亏损/微利年份不再显示无意义的空白横杠 `—`，而是自适应展示「扭亏为盈」、「由盈转亏」、「持续减亏」、「持续亏损」。
- **A 股与港股回购及原生总股本数据全链路打通（P1.1）**：
  - **数据管线接入**：A 股资产负债表 `实收资本/股本` 映射至 `CommonStockSharesOutstanding`（面值 1 元即股数），东方财富回购明细接口聚合写入 `ShareRepurchaseAmt`；港股接入官方最新已发行股本与现金流量表回购股份；
  - **四宫格与趋势表高亮联动**：计算并展示带显式币种符号的 5 年累计回购额（如贵州茅台 `¥90.0亿`、五粮液 `¥11.0亿`、泡泡玛特 `HK$10.5亿`、苹果 `$4,385.8亿`），5年股本注销 $\le -3\%$ 触发「🔥 注销回购」，有回购触发「持续回购」。
- **特殊行业财务模型自适应与排雷系统（P2）**：
  - **5 大行业模型感知引擎（`detectSectorModel`）**：跨市场精准识别 `银行与保险`、`公用事业特许基建`、`房地产`、`强周期资源` 与 `标准工商业`；
  - **彻底消除虚假红灯与指标失真**：
    - **银行保险（MFC、BAC、招商银行）**：消除 95% 负债率误报高杠杆，显示「特许资金杠杆（中性标） · 负债主体为客户存款/保单准备金」；造血力显示「资金营运与息差 · 特许资金运作」；
    - **公用事业（长江电力）**：呈现「特许现金造血 · 特许现金奶牛」与「长期特许项目债 · 充沛现金流覆盖利息」；
    - **房地产（我爱我家、英皇国际）**：Metric 4 标注说明「包含大量预收购房款(合同负债) · 重点关注真实现金短债比」；
    - **强周期资源（西方石油、中国神华）**：Metric 1 提示「周期起伏回报」，Metric 4 提示「穿越大宗低谷安全垫」；
  - **强周期景气高点“低 PE 陷阱”排雷 Banner**：针对大宗高位爆发期看似极低的 PE（$\le 9\text{x}$ 且 5年 ROE $\ge 18\%$），自动弹出 Apple 风格琥珀色排雷警告 Banner，警示周期高点低 PE 伴随下行期利润骤降风险，杜绝盲目抄底；
  - **会计准则口径突变平滑（IFRS 17）**：险企新准则导致 5 年营收表观腰斩时自适应标注 `会计准则口径调整 (IFRS 17)`；
  - **标准工商业科技/消费零回归**：苹果（AAPL）、茅台（600519）完全保持原有纯正分析逻辑。
...
- **DVL 页面左上角多代码切换去噪（纯 Ticker 视觉化）**：
  - 针对分级股权 / 多 Ticker 标的（如 Alphabet GOOG/GOOGL），左上角分段胶囊按钮（`.vl-ticker-pill-btn`）全面移除 `Class A` / `Class C` 等冗余文字标注，直接呈现纯粹的代码标识（`GOOG`、`GOOGL`），视觉体验与普通单 Ticker 标的（`.vl-ticker-badge`）保持高度纯净统一；
  - 股份类别信息（Class Label）下沉至按钮 HTML `title` 悬停提示中，保证辅助可访问性与信息无损。
- **大师持仓卡片仓位去噪与极简聚焦**：
  - 彻底移除卡片中「仓位」数值右侧的多 Ticker 组合标签（如 `[GOOGL + GOOG]`），保证 220px 紧凑卡片内的核心指标区纯净不拥挤；
  - 多类别的持股拆解与交易明细继续统一由页面下方的「大师持仓（13F 全量历史明细）」表格完整承载。
- **大师持仓卡片新增基金管理规模（AUM）展示**：
  - 在卡片底部构筑双端对齐 Footer：左下角显示所属机构简称（如“喜马拉雅资本”、“伯克希尔哈撒韦”），右下角展示对应机构的最新 13F 可报告组合总市值（如 `AUM $37.0亿`、`AUM $2,992.5亿`）；
  - 为读者在审视持仓绝对金额与仓位占比时，即时建立组合层面的宏观权重认知。
- **大师持仓卡片多级智能排序重构与容量升级至 Top 6**：
  - 重构 `getValueLineData` 中的卡片排序算法为「在持状态优先（Active > SoldOut） > 报告季度倒序（最新优先） > 持仓占比降序（权重从高到低） > 持仓金额降序」；
  - 彻底根治此前单纯按 `weightPct` 降序导致多年前已清仓历史标的占满卡片、最新季度新进大师（如阿里巴巴 BABA 中段永平 2026Q2 新建仓被挤出前 4 截断）的致命排序缺陷；
  - 将卡片容量由 Top 4 扩容至 **Top 6**，完美契合桌面端每行 3 列的栅格排版，形成工整的双行矩阵（3 + 3），杜绝第二行单卡孤悬的不对称现象，同时保证苹果 (AAPL) 等共识重仓标的全部在持大师全员呈现。
- **Alpha 投资人 Jorge Paulo Lemann 命名标准化**：
  - 将投资人档案 `Filer` 表与 `MasterProfile` 中的中文名“豪尔赫·保罗·雷曼”统一订正为其标准英文名 `Jorge Paulo Lemann`，对齐全体 Alpha 投资人的英文命名标准规范。

### v0.45.6 变更（2026-09-23）

- **公司详情页初始滚动位置归零修复**：
  - 修复因 `CompanySectionTabs` 组件 mount 时执行 `btn.scrollIntoView` 导致浏览器自动将全局窗口垂直滚动至页面中间（深度分析锚点）的交互缺陷；
  - 改用 `navRef.current.scrollTo` 仅限局部水平滑动，并注入 `isFirstMountRef` 守卫，保证页面打开时 100% 保持在页面顶部（`window.scrollY = 0`）。
- **大师持仓卡片股票类别标签统一（「要么不写，要写也是直接写 Ticker」）**：
  - 根治单 Ticker 公司（如戴尔 DELL 13F 中带有重组遗留的 `CL C`）机械渲染出无意义类别徽章的问题；
  - 建立统一逻辑：仅有单一 Ticker 的标的全面隐藏类别徽章保持界面清爽，多 Ticker 复合标的（如 Alphabet GOOG/GOOGL）直接展示真实交易 Ticker 组合（`GOOG + GOOGL`），彻底弃用生硬的法律类名。
- **大师持仓卡片新增「最新季度动作」指标**：
  - 在卡片原有的「仓位」与「金额」后正式加入第 3 列「动作」（新进 / 增持 ↑ +XX% / 减持 ↓ -XX% / 持平 / 清仓），真实呈现该投资人在其持仓历史最近一个 13F 季度的实际调仓动态；
  - 优化单元格标签排版与 `white-space: nowrap` 约束，杜绝金额或动作字符异常折行。

### v0.45.5 变更（2026-09-23）

- **公司详情页移动端全宽自适应修复（彻底根治右侧截断）**：
  - **CSS Grid 轨道膨胀根本原因根治**：定位并修复了 `.company-wrap` 与 `.dvl-wrap` 缺乏 `grid-template-columns: minmax(0, 1fr)` 的缺陷。原先 CSS Grid 默认轨道宽度为 `auto`，内部 540px 的财务矩阵表（`.vl-matrix-table`）导致外层网格轨道被直接撑大到 642.8px，超出手机视口（375px~390px）的部分被外层 `overflow-x: clip` 截断约 250px。通过设置 `minmax(0, 1fr)` 强制网格轨道收敛至视口，杜绝轨道溢出。
  - **CSS 层叠顺序与弹性约束理顺**：修正了 `globals.css` 中 `@media (max-width: 640px)` 被桌面端选择器权重逆向覆盖的问题，将移动端响应式规则统一下沉到底部；全面为 `.vl-card-identity`、`.vl-card-pricing`、`.vl-company-overview-pane` 等子区块补全 `min-width: 0; max-width: 100%; box-sizing: border-box;`。
- **移动端深度投研 7 大维度交互体验全面升级（方案 A）**：
  - **自动平滑居中锁定（Auto-centering）**：选中的维度标签在点击或切换时，自动调用 `scrollIntoView({ inline: "center", behavior: "smooth" })` 居中对齐，前后相邻标签露出一半，视觉对称且自然。
  - **边缘雾化渐隐指示（Fade Mask）**：监听水平滚动边界，只要右侧或左侧存在未露出的 Tab，边缘即自动呈现半透明光晕渐变，提供直观的滑动视觉线索。
  - **「全景矩阵 ⊞ 7」快捷入口与 iOS 原生毛玻璃抽屉（Bottom Sheet）**：Tab 栏右侧固定常驻 Apple 风格胶囊按钮 `[ ⊞ 7 ]`，点击从底部升起毛玻璃半屏抽屉，以双列卡片矩阵清晰罗列全部 7 个投研维度的序号（`01`~`07`）、名称与副标题描述（如商业模式九宫格画布、核心三张表与杜邦分解等）及当前阅读标记，支持一键瞬切直达。
...
- **公司目录严格状态机收口与孤儿数据清理**：
  - **严格状态机收敛**：`src/app/company/page.tsx` 彻底移除 `|| row._count.financials > 0` 软判定，全面绑定 `metadata.onboardPhase >= 1`。杜绝财务数据已抓取但商业概览超时的半成品标的（如 `NSC`、`PURR`、`GBTC`）偷跑进入已完成区，确保前台已完成区的所有公司均具备完整闭环的 5 年财报、股价走势与 3 句话业务概览。
  - **物理清理孤儿脏数据**：从数据库中物理删除历史 13F 权证导入生成的 0 关联孤儿空壳 Entity `GRABW`（CIK `0001855612`），前台「待完善」列表精准收口至 11 家未完成标的（10 家批处理未完成 + 1 家无财报事实存量）。
...
- **Onboard Phase 1 性能优化、生命周期治理与批量稳定性**：
  ...
  - **全量批处理战报与失败标的台账（223/233 完成）**：
    - mini 批处理跑完 233 家有效待处理公司，成功 223 家（全站有财报公司由 319 家跃升至 544 家，Phase 1 规模从 157 家跃升至 379 家，存量 Phase 0 缩减至 95 家）。
    - 剩余 10 家失败标的分类归档并入库记录：
      - *类别 A（5 家）：因 DeepSeek API 偶发超时/503 失败，已具备基础数据，直接单独重跑即愈*：`NSC`（诺福克南方，财务已入库仅概览超时）、`YOU`（Clear Secure，翻译 503）、`TONX`（TON Strategy，翻译 503）、`PURR`（Hyperliquid，概览 503）、`GBTC`（灰度，概览 503）。
      - *类别 B（5 家）：特殊标的/非 10-K 报表结构，需针对性处置*：`CNI`（加拿大国铁，40-F 申报人，US-GAAP 为空）、`SPCX`（SpaceX，未上市）、`FRVO`（Fervo Energy，未上市）、`PSUS`（潘兴广场，封闭式基金）、`GL`（Globe Life，保险财报口径核对）。详细任务见 `TODO.md` P0 ⑮。
...
### v0.44.12 变更（2026-09-04）

- **控制台 `/dashboard` 移动端响应式重构与防溢出优化**：
  - 针对 360px ~ 430px 多种手机视口（Android Small、iPhone SE/13/14/15/Pro Max）全面重构控制台布局；
  - 将 `.dashboard-grid` 媒体查询轨道调整为 `minmax(0, 1fr)`，解决卡片被内部未截断元素撑宽导致的左右晃荡；
  - 为 `.dashboard-workspace-item`、`.dashboard-account-row`、`.dashboard-password-row`、`.dashboard-input-field`、`.dashboard-inline-form` 等全量容器补充 `box-sizing: border-box; min-width: 0; max-width: 100%`；
  - 优化修改密码行与修改邮箱/昵称表单在移动端上的自动流式排版，防止右侧操作按钮溢出；
  - 增加 `< 640px` 紧凑移动端专属样式，调整内边距、头像与字体大小，工作区卡片描述两行平滑自适应折行。
- **AI 对话框输入框去重聚焦内框**：
  - 清除在聚焦或输入文字时 `.chat-input` 内部 `<textarea>` 出现的内围多余矩形边框与残留双层阴影；
  - 统一由外层胶囊 `.chat-input-bar:focus-within` 呈现柔和统一的 Apple 风格聚焦高亮，内部文字区域无额外方框嵌套，提升全屏 `/agent` 页面、大师对话弹窗、洞见解读面板等多场景的文本输入体验。

### v0.44.11 变更（2026-09-04）

- **移动端多页面横向溢出与左右晃荡修复**：
  - 排查并修复 `/login` 登录注册窗口在移动端因 `.login-card` 缺失 `box-sizing: border-box` 导致盒模型总宽度超出视口（`100% + 42px`）、触发全屏左右晃荡的问题。
  - 排查并修复 `/master` 大师页面卡片右偏与截断问题：CSS Grid 轨道 `1fr` 隐含 `minmax(auto, 1fr)` 导致包含超长英文名/机构名的 Alpha 投资人卡片被 `min-content` 撑宽（352px+），叠加外层 padding 导致卡片向右侧视口外溢出 40px+。通过将网格轨道调整为 `minmax(0, 1fr)`、卡片多层级补充 `box-sizing: border-box; min-width: 0; max-width: 100%`、标题与机构名文本截断省略，并在 640px 移动端下将外层内边距调整为 `2rem 1rem 3.5rem`、卡片内边距调整为 `1.1rem 1rem`，彻底消除 360px ~ 430px 屏幕上的偏右、截断与溢出。
  - 全局主容器 `.site-main` 增加 `overflow-x: hidden; overflow-y: auto;`，页面级增加 `overflow-x: clip;` 严防视口级横向滑动。
- **洞见划词工具条与高光分享文案微调**：
  - 将 `/insights/[slug]` 划词浮动条按钮文案由「问 AI 这段」精简为「AI解读」，在移动端与桌面端呈现更轻量紧凑。
  - 调整「高光分享」复制模版排版结构：改为引用段落置首、文章标题与出处居中、品牌标语置尾（`【Value Tribe · 价值部落】买股票就是买公司。用投资大师的框架深度理解一家公司。`）。同步更新单元测试。
...
- **`InsightPost.source`/`author` 数据契约收紧（2026-08-28）**：用户报告两篇刚发布的文章（Meta、JOYY 投资问答录）在 `/insights` 按"Buffett Tribe"过滤时不可见——排查发现两条记录的 `source` 是 `null`（frontmatter 漏填），而列表/详情页的 `post.source || "Buffett Tribe"` 只是显示兜底，真正的过滤查询对 `source` 做精确匹配，`null` 永远匹配不上任何胶囊，即使显示上看起来"已经是 Buffett Tribe"。顺手查了全库：`author` 字段有 14 条历史记录残留未剥离的 Obsidian 双链语法（如 `"[[Acquired]]"`）、5 条为空、34 条是宽泛的制作公司名 `"Colossus"`（掩盖了旗下具体节目 "Invest Like the Best"/"Business Breakdowns" 的区分度）——已按用户拍板的方案做一次性历史回填（34 条 Colossus → 各自的 source 值；5 条空值 → 各自的 source 值；14 条方括号 → 剥离），回填后 `author` 在全部 87 篇文章上都等于 `source`。**据此拍板新的字段契约**：`source` 是纯文本展示名（不是 URL，URL 单独存 `sourceUrl`），**导入时为空即拒绝导入**（不静默置空、不猜）；`author` 允许为空，为空时脚本自动填成 `source` 的值；`sourceUrl` 不空时是 `source` 文本背后的可点击链接；`/insights` 列表页胶囊过滤按 `source`（原样不变）；**详情页不再展示/读取 `author`** 字段（`src/app/insights/[slug]/page.tsx` 删除了原本"author 与 source 不同时额外显示一行"的逻辑和 select）。实现：新增共享函数 `resolveInsightSourceAndAuthor()`（`src/lib/insights.ts`，含 `stripWikiLinkBrackets()` 剥离双链语法、`KNOWN_SHOW_NAMES` 白名单——原本内联在 `scripts/lib/insight-import.ts` 里，现在两处调用方共用一份逻辑），`scripts/import-insight.ts`（`npm run import:insight`）调用后已改为空 `source` 直接抛错退出（`exit 1`）。**已知未处理的口子**：`src/app/api/insights/posts/route.ts`（token 鉴权的 HTTP 导入接口，供外部自动化用）有自己独立的一套 source/author 解析逻辑，没有接入新的 `resolveInsightSourceAndAuthor()`，同样可能导入出空 `source`——发现但未修，等下次专门排期。
...
| 公司股价 `StockPrice` | `npm run import:company-stock-prices:yf`（`--market cn,hk`/`us` 拆市场，不传 `--start` 时按每支 ticker 自己的最后记录日期续跑） | 每周，锚定 mini crontab（见下）：**周六 12:00** 北京时间跑 cn,hk（2026-09-11 从 01:00 后移，原因见右栏 ⑥），周日 01:00 跑 us | ❌ 待建（"每家公司最新价格日期是否在 7 天内"）——这次只把更新任务接了 cron，巡检按用户要求另开话题未做 | ✅ **2026-08-29 接入 air7 cron，2026-08-30 迁移到 mini**——是全站第一条真正跑起来的定时任务，不再只是"约定"。上线时顺带发现并修复：①"固定最近 N 天"窗口会让停更超过 N 天的 ticker 永久漏缺口（当时 159/625 支已停更 >2 周，AAPL 停更 3 个月），改成 per-ticker 从自己最后记录日期续跑；② Prisma 的 `NOT: { metadata: { path, equals } }` JSON 过滤在 key 不存在时是 NULL 语义，会把所有行误判不匹配整体排除（不是只排除目标行），改成 JS 侧过滤；③ 56 支历史 13F 持仓 ticker 因收购/私有化/改名永久从 yfinance 404，已用新脚本 `mark-delisted-tickers.ts` 标记 `Entity.metadata.delisted`/`Security.metadata.delisted`（两层都要标，见该脚本注释里 Howard Hughes 重组的具体案例）；④ **2026-08-30 发现并修复一个会让价格数据永久冻结的 checkpoint 设计缺陷**——`fetch-stock-prices-yf.py` 的 skip 判断只比对参数签名，而 `--start` 是从当前 `StockPrice` 最大日期反推的，一个 ticker 一旦被跳过一次，它自己的最大日期就不再前进，下一次（哪怕一周后）算出的签名完全相同，于是永远被跳过；已加 20 小时过期窗口修复（同批次崩溃重跑仍会跳过，但保证跨周一定重新检查），详见 `scripts/README.md` §13。⑥ **2026-09-11 修掉一个每周稳定丢一个交易日的 bug**：`cn,hk` 原本跑在周六 01:00，距周五亚洲收盘仅约 10 小时，Yahoo 尚未结算当日日线，最后一根 bar 的 `close` 是 NaN，被 `normalizeYahooChartResponse` 静默跳过——A股/港股因此永远比美股少一个交易日（用户从 `/agent` 资产组合面板看到 A股 9/3、美股 9/4 发现的）。日志里一直有证据（同一 ticker `Replacing ... (8 rows)` 对 `checkpoint saved (9 rows ... -> 09-04)`，14 支亚洲标的无一例外），美股跑在周日 01:00、结算余量 21 小时所以从没踩到。cron 改到周六 12:00（余量约 21 小时，与美股相当），并给跳过行加了 `WARNING` 日志。`RESUME_OVERLAP_DAYS = 3` 让这个洞每周自愈，所以历史无永久缺口（核查过 6 月以来全部亚洲标的，工作日缺口只有 6/19 端午、7/1 香港回归日两个真实休市日）。同批新增 `check:stale-tickers` 巡检「还能返回数据但尾部冻结」的退市 ticker（与已有的 404 形态不同：被收购后 yfinance 往往仍应答，周度 cron 每次都报 complete），刻意只报告不自动标记——`QLYS` 在 9/6 那次运行里报 possibly delisted，四天后正常交易，误标的代价比漏标高；据此核实并标记了 `CWAN`/`OLPX`/`CPRX`/`EA`/`LBRDK` 五支。⑤ 2026-08-30 因 air7（3.4G 内存）跑批量 onboarding 时被无关的另一项目（`pi-matrix`）挤爆内存触发硬重启，`StockPrice`/`onboard:pending` 两条任务一起迁移到 Mac mini（`r129-ai`，32G 内存），air7 只留 `pi-gateway-buffett-tribe`——迁移细节见 `handoff.md`，`CLAUDE.md` 的 Architecture 一节已同步更新 |
...
6. 批量 onboarding（`npm run onboard:pending`，扫描 `Financial` 行数为 0 的公司桩批量补全）：**2026-09-23 在 mini 上完成 Phase 1 极速批量导入**，233 家有效待处理池成功 223 家（全库 636 家中有完整财务指标的公司从 319 家跃升至 544 家，Phase 1 规模达 379 家，存量 Phase 0 缩减至 95 家）。剩余 10 家失败标的（5 家 DeepSeek 503 超时、5 家特殊报表/未上市）已分类建账至 `TODO.md` P0 ⑮ 待后续针对性重跑与补录。
