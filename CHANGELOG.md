...
- **Admin 公司大盘与自动化管线监控面板（/admin/universe）**：
  - 新增管理后台路由 `/admin/universe` 与侧边栏「公司大盘」直达入口（`Building2` 图标）；
...
- **后台自动化 Hourly Phase 1 批处理管线（mini 机器挂载运行）**：
...
### Fixed
- **公司大盘检索表格与徽章视觉规范**：
  - 修复全局样式缺失 `.admin-table` 导致的大盘搜索结果表格排版紧密折行的问题；
...
### Added
- **全市场上市公司全量建册入库（Master Universe Phase 0）**：
  - `Entity` 模型正式引入物理独立状态字段 `onboardPhase Int @default(0)` 并建立 `@@index([market, onboardPhase])` 和 `@@index([onboardPhase])` 索引，作为后续批量自动化任务调度的状态机引擎；
  - 整理并一次性入库 A股、港股、美股全量上市公司基础目录 15,792 家，使数据库底层全量大盘底座达到 **16,435 家**（美股 8,060 · A股 5,569 · 港股 2,806）；
  - 存量历史 643 家已研究公司完成无损回填与状态绑定（Phase 1: 467 家, Phase 2: 162 家, Stubs: 14 家）。
- **全量 16,435 家上市公司模糊检索接口**：
  - 新增 `/api/company/search` 接口，支持按中文名、英文名、拼音简称、证券代码（如 `600519` / `00700` / `AAPL`）在 16,435 家标的池中进行毫秒级模糊匹配；
  - 在 Supabase Postgres 上为 `Entity` 创建 `pg_trgm` GIN 索引，保障超大规模模糊搜索在 300~500ms 内高效返回；
  - 搜索结果按匹配精确度与已建档优先级排序（`Phase 2` > `Phase 1` > `Phase 0`）。

### Changed
- **公司库（/company）页面交互体验重构**：
  - **默认极简展示**：下方「A股 / 港股 / 美股」三大市场区域默认完全隐藏，首屏仅保留页头总数（16,435 家）及「最近更新」（18 家深度建档公司）精选卡片，首屏加载速度从 ~20 秒大幅提升至 ~1 秒；
