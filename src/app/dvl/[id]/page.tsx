...
export default async function DigitalValueLinePage({ params, searchParams }: Props) {
  ...
  const classicCompanyUrl = canonicalUrl.startsWith("/company/")
    ? canonicalUrl
    : `/company/${trimmedId}`;
  ...
  return (
    <div className={`company-page dvl-page ${isEmbed ? "company-page--embed" : ""}`}>
      ...
      <div className="company-wrap dvl-wrap">
        {/* ── Top Navigation & View Indicator ── */}
        <nav className="dvl-top-nav-bar" aria-label="数字价值线导航">
                    ...
                    if (hasReadableArtifact) {
                      return (
                        <Link
                          ...
                          href={`${classicCompanyUrl}/filing/${filing.id}`}
                        >
                          {cardHead}
                        </Link>
                      );
