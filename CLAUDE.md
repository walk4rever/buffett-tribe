...
air7 also hosts a third, unrelated deployable: `/root/cron-job-buffett-tribe` (deployed via `scripts/deploy-cron-job.sh`, a full rsync of this repo — not a trimmed subset, since different `scripts/*.ts` reach into different `src/lib/*` files with unpredictable transitive deps) is the shared home for this repo's scheduled jobs, currently just the weekly `StockPrice` update (`scripts/cron/update-stock-prices.sh`, registered in air7's crontab — see PRODUCT.md "数据更新节奏与自动化现状" for the StockPrice row and `scripts/README.md` §13 for operational detail). Its `.env.local` (DB URL only) is placed manually on air7 and excluded from rsync, same pattern as pi-gateway's `.pi-agent`.
