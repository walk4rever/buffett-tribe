...
Batch/scheduled jobs that aren't `/agent`'s live traffic run on a Mac mini (`r129-ai`, ssh alias `mini`, 32GB RAM, Tailscale-reachable) — **not** on air7. This moved 2026-08-30 after a batch onboarding run on air7 triggered an OOM-induced hard reboot (a since-removed, unrelated project — `pi-matrix`, a 6-container docker-compose stack — was also crammed onto the same 3.4GB box). air7 now only runs `pi-gateway-buffett-tribe` (the `/agent` chat gateway, which must stay on an always-on cloud host); nothing else buffett-tribe-related remains there — `/root/cron-job-buffett-tribe` and its crontab entries were deleted, `scripts/deploy-cron-job.sh` is unused (kept only as a reference for the rsync+npm-install+venv steps it automated, in case a cloud host is ever needed again). On mini: `~/buffett-tribe` is a full repo checkout (repo + `node_modules` + its own `.venv`, `.env.local` placed manually with the full app env, not just DB URL), with the same two weekly `StockPrice` cron entries air7 used to run (`scripts/cron/update-stock-prices.sh`, see PRODUCT.md "数据更新节奏与自动化现状" and `scripts/README.md` §13) plus the batch onboarding entry point (`npm run onboard:pending`, `scripts/onboard-pending-companies.ts` — sweeps every company Entity with zero `Financial` rows and runs `onboard:company` over it with per-ticker failure isolation; see `handoff.md` for the 2026-08-29/30 session that built this out, including several real bugs it surfaced in the underlying onboarding pipeline). mini's home-network path to Cloudflare R2 has meaningfully higher latency than air7's did (~4x on a raw TLS handshake), which shows up directly in 10-K import time since each filing section is archived to R2 serially — known, unresolved cost, not a bug.
