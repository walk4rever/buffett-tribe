...
describe("extractTargetSections", () => {
  ...
  it("extracts 20-F sections via bare-digit page footers (EU-combined filers like Ferrari)", () => {
    // EU-style combined "Annual Report and Form 20-F" filers render page
    // numbers as a centered <span> per page instead of "Page N" text, so
    // parsePageNumber() never matches. This locates page boundaries from
    // that footer instead.
    const rows = [
      ["3.", "Key information", "2-3"],
      ["4.", "Information on the company", "4-5"],
    ...
    const footerPage = (n: number, text: string) => `
      <div><p>${text}</p></div>
      <div><div style="text-align:center"><span>${n}</span></div></div>
    `;
    const html = `
      <html>
        <body>
          ...
          ${footerPage(1, "Cover page content, not a target section.")}
          ${footerPage(2, "Risk factor disclosure appears here.")}
          ${footerPage(3, "Capital and indebtedness disclosure appears here.")}
          ${footerPage(4, "Company history disclosure appears here.")}
          ${footerPage(5, "Business overview disclosure appears here.")}
          ${footerPage(6, "Operating review disclosure appears here.")}
          ${footerPage(7, "Director and employee disclosure appears here.")}
          ${footerPage(8, "Major shareholder disclosure appears here.")}
          ${footerPage(9, "Financial information disclosure appears here.")}
          ${footerPage(10, "Additional information disclosure appears here.")}
          ${footerPage(11, "Market risk disclosure appears here.")}
          ${footerPage(12, "Exhibit disclosure appears here.")}
        </body>
      </html>
    `;
