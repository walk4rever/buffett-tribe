...
// real OpenAI embeddings API (no mocks). See TODOS.md "测试体系设计" for why
...
//
// Requires DIRECT_URL and OPENAI_API_KEY (real per-call cost). Skips (not
...
const hasDeps = Boolean(process.env.DIRECT_URL) && Boolean(process.env.OPENAI_API_KEY);

describe.skipIf(!hasDeps)("search_wisdom golden cases (live DB + OpenAI embeddings)", () => {
