/**
 * Seed business canvas data for AAPL / KO / MCO.
 *
 * Usage: npx tsx scripts/seed-business-canvas.ts
 */

import prisma from "../src/lib/prisma";
import { getCompanyByTicker } from "../src/lib/company-data";

const canvasData: Record<
  string,
  {
...
async function main() {
  for (const [ticker, canvas] of Object.entries(canvasData)) {
    const company = await getCompanyByTicker(ticker);
    if (!company) {
      console.warn(`Company not found: ${ticker}`);
      continue;
    }

    await prisma.businessCanvas.upsert({
      where: { entityId: company.id },
      update: { canvas, source: "llm", version: { increment: 1 } },
      create: { entityId: company.id, canvas, source: "llm" },
    });

    console.log(`✅ Seeded business canvas for ${ticker}`);
  }
}
