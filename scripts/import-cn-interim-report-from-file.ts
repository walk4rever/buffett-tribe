...
type PeriodicRecord = {
  ...
  kind: "cn-interim-report" | "cn-quarterly-report";
  form: string;
  title: string;
  url: string;
  pdfPath: string;
...
async function main() {
  ...
  if (!filePath || !ticker || !code || !market) {
    console.error("Usage: tsx import-cn-interim-report-from-file.ts <json-file> --ticker <T> --code <C> --market cn");
    process.exit(1);
  }
  ...
  for (const report of reports) {
    ...
    await archiveFilingArtifact(db, {
      ...
      originalName: `${code}_${report.periodYear}_${qLabel.toUpperCase()}.pdf`,
