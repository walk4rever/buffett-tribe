...
CN (A-share) mapping is NOT implemented yet — akshare's stock_financial_
report_sina() returns a different (wide, Chinese-column-header) shape that
needs its own verified mapping table before it's trusted. See TODOS.md.
"""
...
def main() -> int:
    ...
    if args.market == "cn":
        print(
            "A-share (--market cn) line-item mapping is not implemented yet — "
            "akshare's stock_financial_report_sina() needs a verified Chinese-"
            "column-header mapping table first (see TODOS.md). Refusing to "
